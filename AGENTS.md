...
- **No `any`.** Untrusted external input (MQTT payloads, request bodies) gets a TypeBox schema plus a runtime check; internally decoded, structurally guaranteed data uses plain TypeScript interfaces. **Outgoing response shapes are defined once as a TypeBox model in `backend/src/schemas/`, and their TypeScript type is derived with `Static<>` (`typeof Schema.static`) — never written a second time as a parallel interface.** The frontend derives the same types from those models through Elysia Eden, so a shape has exactly one definition across both apps. This clause is what stops the duplication that made this rule necessary: the vehicle shape was previously hand-written three times, in `gtfs/enrich.ts`, `routes/realtime.ts`, and `frontend/lib/types.ts`, with nothing linking them.
...
> [!IMPORTANT]
> Both apps have automated tests, but coverage is thin and full of gaps. The backend has a `bun:test` suite (`backend/test/`, run via `bun run test`, gated in CI alongside `bun run check`) covering the realtime SSE transport (`Broadcaster`, `makeSseStream`, per-feed staleness), the ramp reservation DB/route/bridge/proximity lifecycle, `gtfs/time.ts`, `services/transit/arrivals.ts`, GTFS static parsing (`gtfs/static.ts`'s `parseGtfsZip`), and GTFS-Realtime protobuf decoding (`gtfs/realtime.ts`'s `decodeFeedMessage`) against small checked-in fixtures (`backend/test/fixtures/`) — `gtfs/realtime.test.ts` still only covers `startPushLoop`'s cadence separately from decoding. The frontend has an end-to-end Playwright suite (`frontend/e2e/`, run via `bun run test:e2e`, CI-gated) covering the map and ramp-reservation flows, plus a small `bun:test` unit suite (`frontend/test/lib/*.test.ts`, run via `bun run test`) that so far only covers `lib/config.ts`'s API-base resolution, the ramp reservation-diff logic, and the trip ETA merge. Frontend Biome now covers `.tsx` as well as `.ts`, so `bun run check` lints and formats React components too — it did not until recently, which is how 78 findings, including 13 `useExhaustiveDependencies` and 11 accessibility violations, accumulated unseen. Treat all these gaps as known rather than a reason to trust a change because it compiles: verify behavior by exercising it, and prefer adding a test to leaving one absent. When a module resists testing, see [`backend/AGENTS.md`](backend/AGENTS.md)'s Rules section for the DI-factory / pure-function extraction heuristic before reaching for a mock.
