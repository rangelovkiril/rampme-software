...
**One combined rule, not one per route, on a 10s window, not 60s.** This account's plan tier allows exactly one rule in the `http_ratelimit` phase entry-point ruleset (API error code 50001) and only a 10-second counting period, not 60 ("not entitled to use the period 60, can only use a period among [10]") — neither limit is documented anywhere checkable beforehand; both surfaced only via the actual apply. `POST /ramp/reserve` and `DELETE /ramp/reserve/:id` share one rule and one counter: 5 requests per 10 seconds per IP **combined** across create and cancel, the closest available approximation of the original 20-requests-per-60s design intent (`5/10s` extrapolates to `30/min` if sustained, in the same order of magnitude as the original 20/min, and a shorter window is if anything stricter against a bursty scripted client since it can't bank allowance across a full minute). Rationale for the original number stands: a real rider creates at most a couple of reservations per sitting (`MAX_ACTIVE = 2` already caps concurrent ones), and headroom for a conference room sharing one public IP was the reason 20/minute wasn't set lower to begin with — 5/10s keeps that same headroom at a finer granularity.
