...
[Cloudflare's zone-level rate limiting rule syntax/quota for this plan tier is unverified] → Mitigation: the tasks below include confirming the actual rule syntax and any plan-tier limits (number of custom rules, minimum window) against the live dashboard/docs before writing the `tofu` resource, rather than assuming a specific Cloudflare product tier's capabilities here.
