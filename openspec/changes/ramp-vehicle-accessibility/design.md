...
**The scheduled job runs the backend's own image, not new fleet-repo logic.** The crawl (trinmo fleet-list → per-model detail pages → filter by `status: "В движение"` and expected type → join against the curated model table) is application logic that must stay versioned next to the curated table it joins against, so it ships as a script in this repo (`backend/scripts/refresh-accessibility.ts`), compiled to its own standalone binary (`bun build --compile`, same as `server`) and shipped in the same image under a different entrypoint (`./refresh-accessibility`). `fleet`'s `CronJob` overrides the container command to that binary instead of building or running anything of its own; it only owns the schedule and the mounted volume, not the join logic. (An earlier draft of this decision assumed the CronJob could just `bun run scripts/refresh-accessibility.ts` against the already-built production image — that image only ships a compiled `server` binary with no bun runtime or source tree in it, so that would have failed the first time anyone tried it. Compiling the script the same way `server` is built closes that gap.)
...
**The crawl's output gets a one-time bootstrap seed baked into the Docker image; the `fleet` CronJob is the only ongoing refresh mechanism, not "rebuild the image to update."** `model -> low_floor` was already effectively "in the container" for free the moment it's a checked-in source file — no cron ever needed there, and no ongoing update story needed either since it's hand-curated and rarely changes. The volatile half (`inventory -> model`) is different: it needs a real, code-change-free refresh path, which is exactly what the CronJob writing to a mounted volume gives — the API process rereads the file on its own timer, no PR, no rebuild, no deploy. Baking `gtfs/vehicle-accessibility.seed.json` into the image (via a `Dockerfile` `COPY` to `RAMP_ACCESSIBILITY_DATA_PATH`'s default location) exists **only** so a fresh deploy never starts from zero data before the CronJob's volume has run once — it is explicitly not a substitute for the CronJob, and re-running the script and committing a new seed is not the intended way to keep this fresh going forward. Once the CronJob's volume is mounted and populated, it shadows this file and becomes the sole source of truth (see the volume-mount-shadowing risk below for sequencing that transition safely).
