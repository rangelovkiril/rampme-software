...
export default function LiveLocation({ active, onError }: LiveLocationProps) {
  ...
  useEffect(() => {
    ...
    if (!window.isSecureContext) {
      console.warn(
        "[location] Insecure context — geolocation requires HTTPS or localhost.",
      );
      notifyError(
        0,
        "Location requires a secure context. Open the app on localhost or HTTPS.",
      );
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        console.log(
          `[location] Initial fix: ${pos.coords.latitude.toFixed(5)}, ${pos.coords.longitude.toFixed(5)} ±${Math.round(pos.coords.accuracy)}m`,
        );
        applyPosition(pos);
      },
      (err) => {
        console.warn(
          `[location] getCurrentPosition error: code=${err.code} — ${err.message}`,
        );
        if (err.code === 1) {
          notifyError(
            1,
    ...
    watchRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        console.log(
          `[location] Position update: ${pos.coords.latitude.toFixed(5)}, ${pos.coords.longitude.toFixed(5)} ±${Math.round(pos.coords.accuracy)}m`,
        );
        applyPosition(pos);
      },
      (err) => {
        console.warn(
          `[location] watchPosition error: code=${err.code} — ${err.message}`,
        );
        if (err.code === 1) {
          notifyError(
            1,
