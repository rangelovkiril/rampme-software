...
export default function Map() {
  ...
  const handleVehicleSelect = useCallback((v: Vehicle) => {
    ...
    // VehiclesLayer handles flyTo when selectedVehicleId changes
  }, [])
  ...
  const handleVehicleOpen = useCallback(async (vehicleId: string) => {
    ...
    // Fetch current vehicle position and fly to it (selectedVehicleId may not change if already selected)
    try {
      ...
      if (v && Number.isFinite(v.lat) && Number.isFinite(v.lng)) {
        mapRef.current?.flyTo([v.lat, v.lng], Math.max(mapRef.current.getZoom(), 16), { animate: true, duration: 0.8 })
      }
    } catch {}
  }, [])
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      >
        ...
        <RouteLinesLayer routeId={selectedRoute?.routeId ?? null} routeType={selectedRoute?.routeType ?? null} autoFit={selectedVehicle === null} />
      ...
      <VehicleTripSheet
        ...
        onClose={() => { setSelectedVehicle(null); setCompactSheet(false) }}
