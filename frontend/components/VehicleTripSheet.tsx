...
export default function VehicleTripSheet({
  vehicle,
  onClose,
}: VehicleTripSheetProps) {
  const [trip, setTrip] = useState<TripData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const fetchTrip = useCallback(
    async (vehicleId: string, isInitial: boolean) => {
    ...
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-920 flex justify-center px-0 sm:px-4">
      ...
      >
        ...
        <div className="max-h-[54vh] overflow-y-auto px-4 pb-4">
          ...
          {!loading && error && (
            <p className="py-3 text-sm" style={{ color: "#ef4444" }}>
              {error}
            </p>
          )}
          ...
          {!loading &&
            ...
            trip.stops.length > 0 &&
            (() => {
              const firstUpcomingIdx = trip.stops.findIndex(
                (s) => s.status !== "departed",
              );
              const startIdx =
                firstUpcomingIdx > 0
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-1 items-start justify-between gap-2 pb-3">
                          ...
                          >
                            <p
                              className={`text-sm ${isDeparted ? "" : "font-semibold"}`}
                            >
                              {stop.stop_name}
                            </p>
                            <p
                          ...
                          {!isDeparted && (
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() =>
                                  alert(
                                    `Ramp requested for ${routeName} at ${stop.stop_name}!`,
                                  )
                                }
