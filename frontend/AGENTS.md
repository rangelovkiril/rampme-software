...
| `NEXT_PUBLIC_CARTO_API_KEY` | _(unset)_ | Carto Basemaps API key, appended as a `?key=` query param to the `dark`/`light` `TileLayer` URLs in `components/Map.tsx`. Inlined into the public bundle like every `NEXT_PUBLIC_*` var, so it can't be treated as a secret — CARTO's own domain-restriction on the key is the actual control. In CI it comes from the `CARTO_API_KEY` repository secret, set as `env:` on the `build` job in `.github/workflows/frontend.yaml`, not a Cloudflare Pages project setting (this pipeline builds in GitHub Actions and only uploads the static output via `wrangler pages deploy`). |
