...
Това хранилище съдържа три приложения:
...
- **`hw-sim/`** е самостоятелен симулатор на хардуера на рампата: вграден MQTT брокер плюс скриптируема машина на състоянията за всяко возило, използвана за упражняване на жизнения цикъл резервация-разгъване без физически хардуер.
...
Backend-ът стартира без брокер: без зададен `MQTT_URL` логва, че MQTT се прескача, и обслужва всичко освен хардуерния път. За да се упражни жизненият цикъл на рампата без физически хардуер, пусни [`hw-sim`](../hw-sim/AGENTS.md) и насочи `MQTT_URL` на backend-а към него.
