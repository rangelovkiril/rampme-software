...
- **Vehicle accessibility** has two datasets with very different volatility, deliberately kept separate. `gtfs/model-accessibility.json` is hand-curated (model -> low_floor), reviewed like any other source change, and barely ever changes — it stays here. The volatile half (inventory -> model, crowd-sourced from trinmo.org) is rebuilt weekly by a small dependency-free Node script published as a GitHub gist (linked from the `fleet` repo's `accessibility-refresh` workflow, which fetches this repo's `model-accessibility.json` over HTTP for the join and commits the result there as a `ConfigMap`), not by anything in this repo — it's simple enough (fetch, filter, join, write) that it doesn't need this codebase's TypeScript/TypeBox/test machinery, and living outside both repos means it can be edited without a PR in either. Flux applies fleet's commit like any other change, no in-cluster CronJob or RBAC. `gtfs/accessibility.ts`'s loader just reads `RAMP_ACCESSIBILITY_DATA_PATH` and reloads on an interval — it has no idea the file's origin is a Git-committed ConfigMap rather than a local edit. `vehicle-accessibility.seed.json` (checked in, baked into the Docker image) is only a fallback default for the image outside this specific fleet topology.
...
| `RAMP_ACCESSIBILITY_DATA_PATH` | `./data/vehicle-accessibility.json` | Path to the vehicle wheelchair-ramp accessibility reference dataset, rebuilt out-of-band by a `fleet`-scheduled script (see `openspec/changes/ramp-vehicle-accessibility`); missing file means every vehicle resolves as unknown |
