...
export function publishNewReservation(r: RampReservation): void {
  getMqtt().publish(cmdTopic(r.vehicle_id), { action: 'new_reservation' })
}

/** Published when a reservation is cancelled by the user. */
export function publishCancelReservation(r: RampReservation): void {
  getMqtt().publish(cmdTopic(r.vehicle_id), { action: 'cancel_reservation' })
}
...
export function publishDeploy(vehicleId: string): void {
  ...
  deployTimeouts.set(
    ...
    setTimeout(() => {
      ...
      if (pending.length > 0) {
        console.log(`[ramp-mqtt] deploy timeout for ${vehicleId} — expired ${pending.length} reservation(s)`)
      }
...
function handleHardwareState(vehicleId: string, payload: HardwareState): void {
  console.log(`[ramp-mqtt] ← ${vehicleId} state=${payload.state}${payload.reason ? ` (${payload.reason})` : ''}`)
