...
export interface RampDb {
  ...
  cancelReservation(id: number, sessionId: string): RampReservation | null
  getSessionReservations(sessionId: string): RampReservation[]
  getVehicleReservations(vehicleId: string): RampReservation[]
  getAllActiveReservations(): RampReservation[]
  setReservationStatus(id: number, status: RampReservation['status']): void
  cleanupOldReservations(): number
}

/**
 * Builds an independent RampDb over its own SQLite connection. Production
 * wires a single instance via initRampDb()/getRampDb(); tests call this
 * directly with dbPath ':memory:' for isolated, fast tests instead of a
 * shared on-disk database.
 */
export function createRampDb(dbPath: string): RampDb {
  ...
  function cancelReservation(id: number, sessionId: string): RampReservation | null {
  ...
  function getSessionReservations(sessionId: string): RampReservation[] {
    return stmts.sessionActive.all({
      $session_id: sessionId,
    }) as RampReservation[]
  }

  function getVehicleReservations(vehicleId: string): RampReservation[] {
    return stmts.vehicleActive.all({
      $vehicle_id: vehicleId,
    }) as RampReservation[]
  }

  function getAllActiveReservations(): RampReservation[] {
    return stmts.allPending.all() as RampReservation[]
  }

  function setReservationStatus(id: number, status: RampReservation['status']): void {
    stmts.setStatus.run({ $id: id, $status: status })
  }

  function cleanupOldReservations(): number {
...
let rampDb: RampDb | null = null

/** Wires the singleton RampDb used in production over the real SQLite file. */
export function initRampDb(dbPath: string): RampDb {
  rampDb = createRampDb(dbPath)
  return rampDb
}

export function getRampDb(): RampDb {
  if (!rampDb) throw new Error('Ramp db not initialized — call initRampDb() first')
  return rampDb
}
